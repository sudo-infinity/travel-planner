...
const resolvers = {
  Query: {
    ...
    // Find a user
    user: async (parent, { username }) => User.findOne({ username }).populate("trips"),
    // Find me
    ...
    // Find all trips
    trips: async () => Trip.find().populate("plans"),
    facts: async () => Fact.find(),
    // Find one trip
    trip: async (parent, { tripId }) => Trip.findOne({ _id: tripId }).populate("plans").populate("facts"),
    // Find one plan
    plan: async (parent, { planId }) => Plan.findOne({ _id: planId }),
    // Find one fact
    fact: async (parent, { factId }) => Fact.findOne({ _id: factId }),
  },
  ...
  Mutation: {
    ...
    removePlan: async (parent, { tripId, planId }) => await Trip.findOneAndUpdate(
        { _id: tripId },
        { $pull: { plans: { $eq: planId } } },
        { new: true }
      ),
    removeFact: async (parent, { tripId, factId }) => await Trip.findOneAndUpdate(
        { _id: tripId },
        { $pull: { facts: { $eq: factId } } },
        { new: true }
      ),
    removeTrip: async (parent, { tripId }) => Trip.findOneAndDelete({ _id: tripId }),
    ...
    updateFact: async (parent, { factId, description }) => await Fact.findByIdAndUpdate(
        { _id: factId },
        {
          $set: {
            description,
